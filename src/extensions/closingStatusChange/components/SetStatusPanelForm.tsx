import { DefaultButton, Dropdown, IDropdownOption, Panel, PanelType, PrimaryButton, Stack, Text, TextField } from 'office-ui-fabric-react';
...
export const SetStatusPanelForm: React.FunctionComponent<ISetStatusPanelFormProps> = (props: React.PropsWithChildren<ISetStatusPanelFormProps>) => {
    ...
    const saveForm = (): void => {
        SPListViewService.saveListItem(props.context.pageContext.list.title, listItem.Id, {
            ...
            DocStatusId: Number(statusField),
            DocStatusText: docStatusList.find(status => status.Id === statusField)?.Title,
            ...
            if (listItem.DocStatusId !== statusField) {
                SPListViewService.saveWorkflowTechnialListItem(props.context, {
                    ...
                    ItemID: listItem.Id,
                    DocumentStatusID: Number(statusField),
        ...
        >
            {
                listItem && listItem.PermissionSetInProgress === 0 ? (
                    ...
                    (
                        <Stack>
                            <Text>A dokumentumon jogosultság állítás van folyamatban, kérjük próbálja meg később szerkeszteni.</Text>
                        </Stack>
                    )
            }
        </Panel>
