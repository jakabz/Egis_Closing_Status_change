...
import { IDocStatus, IListItem, IUpdateListFields, IUpdateTecnicalFields } from "./Types";
...
export class listService {
    ...
    public async saveListItem(listTitle: string, id: number, fields:IUpdateListFields): Promise<IItemUpdateResult> {
    ...
    public async getDocStatusList(context: FieldCustomizerContext): Promise<IDocStatus[]> {
        ...
        const docStatusList: IDocStatus[] = await root.getList(context.pageContext.site.serverRelativeUrl + '/Lists/StatusConfig').items();
        return docStatusList;
    }

    public async saveWorkflowTechnialListItem(context: FieldCustomizerContext, fields: IUpdateTecnicalFields): Promise<IItemAddResult> {
        const root: IWeb = Web(context.pageContext.site.absoluteUrl);
